...
let
  ...
  flakeV1 = pkgs.writeText "flake-v1" (builtins.replaceStrings
    [ "__NIXPKGS_PATH__" "__VERSION__" ]
    [ (toString pkgs.path) "1" ]
    flakeTemplate
  );
  flakeV2 = pkgs.writeText "flake-v2" (builtins.replaceStrings
    [ "__NIXPKGS_PATH__" "__VERSION__" ]
    [ (toString pkgs.path) "2" ]
    flakeTemplate
  );
in

{
  ...
  testScript = { nodes, ... }: ''
    ...
    autoupdate.succeed(
        "age -e -r '{}' -o /var/lib/nixos/secrets/git-ssh-key.age "
        "/var/lib/nixos/secrets/git-ssh-key".format(pubkey)
    )
    ...
    gitserver.succeed(
        "echo '{}' >> ~git/.ssh/authorized_keys".format(ssh_pubkey)
    )
    ...
    assert result == "1", "Expected version 1, got {}".format(result)
